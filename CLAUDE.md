...
- `lifecycle.py` — Rian lifecycle receiver: `POST /api/lifecycle` takes a batch of `{repo_url, first_seen, last_seen, mentions}` and upserts **monotonically** (earliest `first_seen`, latest `last_seen` ever received), so a re-send is a no-op and a partial batch can never move a date the wrong way. Unknown `repo_url`s are counted and returned as `unmatched`, never 4xx'd. Join key is `repo_url` normalised on both sides (lowercase host, no trailing slash, no `.git`). `started_at` = Rian `first_seen` → else GitHub `created_at` (`POST /api/lifecycle/github`, fills NULLs only) → else NULL, with `started_at_source` (`rian|github|manual`); a **manual** value set via `update_project` is never overwritten. Audit row per batch in `lifecycle_runs`. `GET /api/projects/census?year=` and the `project_census()` MCP tool answer "how many did we start this year / how many are live". See `docs/rian-lifecycle-sync-prd.md`
...
- **`created_at` is not a start date.** It records when a project was *registered*, and the register began in March 2026, so every backfilled row reads 2026. Use `started_at` (with `started_at_source`) for when a project actually began; `last_seen` lags by however long since chat exports were last fed to Rian and must never be read as activity.
